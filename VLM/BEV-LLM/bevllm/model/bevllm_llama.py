...
class BevLLMLlamaForCausalLM(LlamaForCausalLM, BevLLMMetaForCausalLM):
    ...
    def __init__(self, config, freeze_qformer = False):
        ...
        self.bev_conv1= nn.Conv2d(in_channels=config.bev_channels, out_channels=768, kernel_size=(1,1), stride = (1, 1) )
        ...
        self.vision_proj_norm.requires_grad_(False)

        self.view_position_embeddings = PositionalEncoding(config.pos_encoding_scale, 768, 6)
        ...
        self.qformer_proj = nn.Linear(768,config.hidden_size)
...
#            print(labels)
    ...
    def init_Qformer(self, num_query_token, vision_width, cross_attention_freq=2):
        pretrained_id = getattr(self.config, "qformer_model_id", "bert-base-uncased")
        encoder_config = BertConfig.from_pretrained(pretrained_id, cache_dir=getattr(self.config, "cache_dir", None))
        ...
        Qformer = BertLMHeadModel.from_pretrained(
            pretrained_id, config=encoder_config, cache_dir=getattr(self.config, "cache_dir", None)
        )
