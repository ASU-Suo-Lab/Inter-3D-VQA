...
def parse_object_type_and_side(text: str) -> Tuple[Optional[str], Optional[str]]:
    norm = normalize_parse_text(text)
    match = re.search(
        r"\b(?:it is a|it is an|the|a|an)\s+(?P<object_type>[a-z0-9_ ]+?)\s+"
        r"(?P<location>located in the center area of the intersection|"
        r"located on the (?:north|south|east|west) side of the intersection|"
        r"in the center area(?: of the intersection)?|"
        r"on the (?:north|south|east|west) side(?: of the intersection)?)",
        norm,
    )
    if not match:
        return None, None
    object_type = normalize_object_type(match.group("object_type"))
    side = extract_side_phrase(match.group("location"))
...
def parse_1_1_3(text: str) -> Dict[str, Any]:
    match = re.match(
        r"the\s+(north|south|east|west)\s+side\s+currently\s+has\s+(\d+|zero|one|two|three|four|five|six|seven|eight|nine|ten)\s+([a-z0-9_ ]+?)\.",
...
def parse_1_2_2(text: str) -> Dict[str, Any]:
    ...
    return {"visibility": norm} if norm else {}


def parse_1_3_1(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    if not norm.startswith("the scene appears to be "):
        return {}
    out: Dict[str, Any] = {"light": {}}
    if "sunny" in norm:
        out["weather"] = "sunny"
    if "daytime" in norm:
        out["time_of_day"] = "daytime"
    glare_match = re.search(r"with strong sun glare in the\s+(north|south|east|west)\s+view", norm)
    out["light"]["sun_glare"] = glare_match is not None
    out["light"]["sun_glare_sides"] = [glare_match.group(1)] if glare_match else []
    return out


def parse_1_3_2(text: str) -> Dict[str, Any]:
    match = re.match(r"the signal is currently\s+(red light|yellow light|green light)\.", normalize_parse_text(text))
    if not match:
        return {}
    return {"signal_state": match.group(1)}


def parse_2_1_1(text: str) -> Dict[str, Any]:
    match = re.match(
        r"the\s+([a-z0-9_ ]+?)\s+is\s+(-?\d+(?:\.\d+)?)\s+m\s+from\s+the\s+(north|south|east|west)\s+stop line\.",
        normalize_parse_text(text),
    )
    if not match:
        return {}
...
def parse_2_1_3(text: str) -> Dict[str, Any]:
    match = re.match(
        r"the\s+([a-z0-9_ ]+?)\s+(?:in the center area|on the (?:north|south|east|west) side)\s+is\s+(-?\d+(?:\.\d+)?)\s+m\s+from\s+the\s+([a-z0-9_ ]+?)\s+(?:in the center area|on the (?:north|south|east|west) side)\.",
        normalize_parse_text(text),
    )
    if not match:
        return {}
...
def parse_2_1_4(text: str) -> Dict[str, Any]:
    match = re.match(
        r"it is a\s+([a-z0-9_ ]+?)\s+on the\s+(north|south|east|west|center)\s+side,\s+(-?\d+(?:\.\d+)?)\s+m away\.",
        normalize_parse_text(text),
    )
    if not match:
        return {}
...
def parse_2_2_1(text: str) -> Dict[str, Any]:
    match = re.match(
        r"the\s+([a-z0-9_ ]+?)\s+on the\s+(north|south|east|west)\s+side\s+is currently in the\s+(left-turn lane|through lane|right-turn lane)\.",
        normalize_parse_text(text),
    )
    if not match:
        return {}
...
def parse_2_2_3(text: str) -> Dict[str, Any]:
    match = re.match(
        r"there are\s+(\d+|zero|one|two|three|four|five|six|seven|eight|nine|ten)\s+queued vehicles?\s+in the\s+(north|south|east|west)\s+(left-turn lane|through lane|right-turn lane)\.",
        normalize_parse_text(text),
    )
    if not match:
        return {}
    return {"count": parse_int_token(match.group(1)), "side": match.group(2), "lane_function": match.group(3)}


def parse_2_2_4(text: str) -> Dict[str, Any]:
    match = re.match(
        r"there are\s+(\d+|zero|one|two|three|four|five|six|seven|eight|nine|ten)\s+vehicles within 5 m behind the\s+(north|south|east|west)\s+stop line\.",
        normalize_parse_text(text),
    )
    if not match:
        return {}
    return {"count": parse_int_token(match.group(1)), "side": match.group(2)}


def parse_2_2_5(text: str) -> Dict[str, Any]:
    match = re.match(
        r"the\s+(left-turn lane|through lane|right-turn lane)\s+on the\s+(north|south|east|west)\s+side\s+currently has the longest queue\.",
        normalize_parse_text(text),
    )
    if not match:
        return {}
...
def parse_3_1_1(text: str) -> Dict[str, Any]:
    match = re.match(
        r"the\s+([a-z0-9_ ]+?)\s+(in the center area|on the (?:north|south|east|west) (?:side|approach))\s+is\s+(accelerating|steady|decelerating)\s+at\s+(-?\d+(?:\.\d+)?)\s+m/s\^2\.",
        normalize_parse_text(text),
    )
    if not match:
        return {}
    return {
        ...
        "accel_state": match.group(3),
        "acceleration": format_float(float(match.group(4))),
    }


def parse_3_1_2(text: str) -> Dict[str, Any]:
    match = re.match(
        r"the\s+([a-z0-9_ ]+?)\s+(in the center area|on the (?:north|south|east|west) (?:side|approach))\s+is\s+([a-z-]+)\s+at a speed of\s+(-?\d+(?:\.\d+)?)\s+m/s\.",
        normalize_parse_text(text),
    )
    if not match:
        return {}
    return {
        "object_type": normalize_object_type(match.group(1)),
        "side": extract_side_phrase(match.group(2)),
        "motion_state": match.group(3),
        "speed": format_float(float(match.group(4))),
    }
...
def parse_3_1_3(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    match = re.match(
        r"the\s+([a-z0-9_ ]+?)\s+(in the center area|on the (?:north|south|east|west) (?:side|approach))\s+is\s+(.+)\.",
        norm,
    )
    if not match:
        return {}
...
def parse_3_1_1_current_motion_state(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    match = re.match(
        r"the\s+([a-z0-9_ ]+?)\s+(in the center area|on the (?:north|south|east|west) (?:side|approach))\s+is\s+([a-z-]+)\s+at\s+(-?\d+(?:\.\d+)?)\s+m/s(?:\s+and\s+(accelerating|steady|decelerating)\s+at\s+(-?\d+(?:\.\d+)?)\s+m/s\^2)?\.",
        norm,
    )
    if not match:
        return {}
    ...
    if match.group(5) is not None and match.group(6) is not None:
        out["accel_state"] = match.group(5)
        out["acceleration"] = format_float(float(match.group(6)))
    return out


def get_discrete_fields_3_1_1_current_motion_state(gt: Dict[str, Any]) -> List[str]:
    fields = ["object_type", "side", "motion_state"]
    if gt.get("accel_state") is not None:
        fields.append("accel_state")
    return fields


def get_numeric_fields_3_1_1_current_motion_state(gt: Dict[str, Any]) -> Dict[str, List[str]]:
    bucket_map = numeric_bucket_map(speed=["speed"])
    if gt.get("acceleration") is not None:
        bucket_map["acceleration"] = ["acceleration"]
    return bucket_map
...
def parse_3_2_2(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    match = re.match(
        r"the\s+([a-z0-9_ ]+?)\s+(?:in the center area|on the (?:north|south|east|west) side)\s+is likely to\s+(.+)\.",
        norm,
    )
    if not match:
        return {}
...
def parse_3_3_2(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    match = re.match(
        r"the\s+(left-turn lane|through lane|right-turn lane)\s+on the\s+(north|south|east|west)\s+side\s+is most likely to form a long queue because it already contains\s+(.+)\.",
        norm,
    )
    if not match:
        return {}
...
def parse_3_4_4(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    match = re.match(
        r"the dominant conflict pattern is\s+([a-z0-9_-]+)\s+(?:on the\s+(north|south|east|west)\s+side|in the center area)\.",
        norm,
    )
    if not match:
        return {}
...
def parse_4_1_2(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    base = re.match(
        r"the\s+(north|south|east|west)\s+side is\s+([a-z ]+?)(?:,\s+with\s+(\d+)\s+vehicles moving and\s+(\d+)\s+stopped)?\.",
        norm,
    )
    if not base:
        return {}
...
def parse_4_1_3(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    if "no clear abnormal behavior is currently dominant." in norm:
        return {"abnormal_or_none": "none"}
    match = re.search(
        r"the most notable current risk is\s+([a-z ]+?)\s+(?:on the\s+(north|south|east|west)\s+side|in the center|around the intersection)\.",
        norm,
    )
    if not match:
        return {}
    ...
    if focus is None:
        focus = "center"
    return {"abnormal_or_none": match.group(1).strip(), "primary_focus": focus}


def parse_4_1_4(text: str) -> Dict[str, Any]:
    imbalance = parse_yes_no(text)
    if imbalance is None:
        return {}
    out = {"has_flow_imbalance": imbalance}
    if imbalance:
        match = re.search(r"the\s+(north|south|east|west)\s+side contains the largest share", normalize_parse_text(text))
        if match:
            out["dominant_side"] = match.group(1)
    return out
...
def parse_4_2_1(text: str) -> Dict[str, Any]:
    ...
    if not risk:
        return out
    match = re.match(
        r"yes,\s+a\s+([a-z0-9_ ]+?)\s+(in the center area of the intersection|on the (?:north|south|east|west) side of the intersection)\s+is still moving at about\s+(-?\d+(?:\.\d+)?)\s+m/s\.",
        normalize_parse_text(text),
    )
    if not match:
        return out
...
def parse_4_2_2(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    match = re.match(
        r"a\s+([a-z- ]+?)\s+exists\s+(?:on the\s+(north|south|east|west)\s+side|in the center area)\.",
        norm,
    )
    if not match:
        return {}
...
def parse_4_3_1(text: str) -> Dict[str, Any]:
    return parse_action_state_exact(text, INTERSECTION_ACTION_BY_TEXT)


def parse_4_3_2(text: str) -> Dict[str, Any]:
    return parse_action_state_exact(text, SIDE_ACTION_BY_TEXT)


def parse_4_3_3(text: str) -> Dict[str, Any]:
    return parse_action_state_exact(text, LANE_ACTION_BY_TEXT)


def parse_4_3_4(text: str) -> Dict[str, Any]:
    norm = normalize_parse_text(text)
    match = re.match(r"the\s+([a-z0-9_ ]+?)\s+(?:in the center area|on the (?:north|south|east|west) side)\s+(.+)", norm)
    if not match:
        return {}
    action_state = None
...
def get_discrete_fields_4_1_4(gt: Dict[str, Any]) -> List[str]:
    fields = ["has_flow_imbalance"]
    if gt.get("has_flow_imbalance"):
        fields.append("dominant_side")
    return fields
...
def get_discrete_fields_1_2_2(gt: Dict[str, Any]) -> List[str]:
    return ["visibility"] if gt.get("visibility") is not None else []
...
V5_TEMPLATE_SPECS: Dict[str, TemplateEvalSpec] = {
    ...
    "1_2_2_visibility": TemplateEvalSpec(parse_1_2_2, discrete_fields=get_discrete_fields_1_2_2),
    "1_3_1_weather": TemplateEvalSpec(
        parse_1_3_1,
        discrete_fields=lambda _gt: ["weather", "time_of_day", "light.sun_glare", "light.sun_glare_sides[*]"],
    ),
    ...
    "3_1_1_accel_state": TemplateEvalSpec(
        parse_3_1_1,
        discrete_fields=lambda _gt: ["object_type", "side", "accel_state"],
        numeric_fields=lambda _gt: numeric_bucket_map(acceleration=["acceleration"]),
    ),
    "3_1_1_current_motion_state": TemplateEvalSpec(
        parse_3_1_1_current_motion_state,
        discrete_fields=get_discrete_fields_3_1_1_current_motion_state,
        numeric_fields=get_numeric_fields_3_1_1_current_motion_state,
    ),
    "3_1_2_vehicle_motion_state": TemplateEvalSpec(
        parse_3_1_2,
        discrete_fields=lambda _gt: ["object_type", "side", "motion_state"],
        numeric_fields=lambda _gt: numeric_bucket_map(speed=["speed"]),
    ),
    "3_1_2_vehicle_maneuver": TemplateEvalSpec(parse_3_1_3, discrete_fields=lambda _gt: ["object_type", "side", "maneuver"]),
    "3_1_3_vehicle_maneuver": TemplateEvalSpec(parse_3_1_3, discrete_fields=lambda _gt: ["object_type", "side", "maneuver"]),
    ...
    "4_1_4_flow_imbalance": TemplateEvalSpec(parse_4_1_4, discrete_fields=get_discrete_fields_4_1_4),
...
def read_predictions(path: Path, sidecar_rows: List[Dict[str, Any]]) -> Dict[str, str]:
    if path.suffix == ".jsonl":
        ...
        if rows and "question_id" in rows[0]:
            ...
            for row in rows:
                ...
                pred = row.get("prediction") or row.get("predict") or row.get("answer") or row.get("response") or row.get("output")
    ...
    if isinstance(payload, list):
        ...
        for row in payload:
            ...
            pred = row.get("prediction") or row.get("predict") or row.get("answer") or row.get("response") or row.get("output")
            require(isinstance(pred, str), f"Missing prediction text for question_id={row['question_id']}")
            out[str(row["question_id"])] = pred
        return out
...
def evaluate_discrete_fields(gt: Dict[str, Any], pred: Dict[str, Any], patterns: Sequence[str]) -> Dict[str, Any]:
    ...
    for field_name in field_names:
        gt_tokens = semantic_value_tokens(field_name, gt_flat.get(field_name))
        pred_tokens = semantic_value_tokens(field_name, pred_flat.get(field_name))
        field_scores[field_name] = token_precision_recall_f1(gt_tokens, pred_tokens)
    ...
    return {
        ...
        "valid": expected,
        "expected": expected,
        "coverage": None if expected == 0 else 1.0,
...
def aggregate_numeric_metric_entries(entries: Sequence[Dict[str, Any]], *, include_field_errors: bool) -> Dict[str, Any]:
    ...
    for entry in entries:
        ...
        entry_field_errors = entry.get("field_errors", {})
        errors.extend(entry_field_errors.values())
        if include_field_errors:
            field_errors.update(entry_field_errors)
    metrics = {
        "mae": mean(errors),
        "rmse": None if not errors else math.sqrt(sum(error * error for error in errors) / len(errors)),
        "valid": len(errors),
        "expected": expected,
        "coverage": None if expected == 0 else len(errors) / expected,
    }
    if include_field_errors:
...
def summarize_rows(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    return {
        ...
        "numeric_error": aggregate_numeric_group(rows),
...
def aggregate_macro(per_group: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for group_name, metric_names in {
        ...
        "numeric_error": ["mae", "rmse", "coverage"],
        "discrete_semantics": ["precision", "recall", "f1"],
    }.items():
        for metric_name in metric_names:
    ...
    for bucket in NUMERIC_ERROR_BUCKETS:
        ...
        for metric_name in ("mae", "rmse", "coverage"):
            values = [
...
def main() -> None:
    ...
    metrics_payload = {
        "metadata": {
            ...
            "evaluation_schema": {
                "text": "normalized_exact_match, bleu_4, rouge_l, bertscore, simcse, weighted_text_score",
                "numeric_error": "mae/rmse over answer-required structured numeric fields with bucketed breakdown",
                "discrete_semantics": "field-level token precision/recall/f1 over answer-required semantic fields",
            },
            "numeric_error_buckets": {
                ...
                "time_headway": "time-headway values when present in answer-required fields",
                "ratio": "ratio-like values when present in answer-required fields",
                "global_3d_xy": "scene/global ground-plane x/y coordinates",
                "image_2d_xy": "image-plane x1/y1 coordinates",
                "waypoint_xy": "future waypoint dx/dy offsets",
            },
        },
        "overall_text": aggregate_text_group(per_sample_rows),
        "overall_numeric_error": aggregate_numeric_group(per_sample_rows),
