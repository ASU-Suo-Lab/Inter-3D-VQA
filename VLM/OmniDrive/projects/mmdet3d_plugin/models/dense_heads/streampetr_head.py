...
class StreamPETRHead(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        if self.output_dims is not None and self.use_can_bus:
    ...
    def post_update_memory(self, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict, rec_can_bus=None):
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        if self.with_ego_pos and self.use_ego_motion:
    ...
    def forward(self, img_metas, pos_embed, **data):
        ...
        if rec_can_bus is not None:
            rec_can_bus = rec_can_bus.unsqueeze(-2)
        out_memory = self.post_update_memory(data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict, rec_can_bus)
