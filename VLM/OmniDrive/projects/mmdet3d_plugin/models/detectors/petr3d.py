...
class Petr3D(MVXTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 export_onnx=False,
                 reset_memory_on_new_scene=False):
        ...
        self.reset_memory_on_new_scene = reset_memory_on_new_scene
    ...
    def _unwrap_meta_value(self, value):
        if hasattr(value, 'data'):
            value = value.data
        return value

    def _to_json_value(self, value):
        value = self._unwrap_meta_value(value)
    ...
    def _to_bool(self, value, default=True):
        ...
        if isinstance(value, (list, tuple)):
            return self._to_bool(value[0], default=default) if value else default
        return bool(value)
    ...
    def forward_test(self, img_metas, rescale, **data):
        ...
        for key in data:
            if key not in ['img', 'input_ids']:
                ...
                data[key] = data[key][0]
        prev_exists = self._to_bool(data.get('prev_exists'), default=True)
        if (not self.test_flag) or (self.reset_memory_on_new_scene and not prev_exists):
            self._reset_temporal_memory()
            self.test_flag = True
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        generated_text = []
        sample_idx = self._to_json_value(img_metas[0]['sample_idx'])
        scene_id = self._to_json_value(img_metas[0].get('scene_id'))
        question_ids = self._to_json_value(img_metas[0].get('question_ids', []))
        questions = self._to_json_value(img_metas[0].get('vlm_labels', []))
        timestamp = self._to_json_value(img_metas[0].get('timestamp'))
        result_path = os.path.join(self.save_path, f'{sample_idx}.json')
        if self.with_lm_head:
            ...
            for i, input_ids in enumerate(data['input_ids'][0]):
                ...
                prediction = self.tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0].strip()
                generated_text.append(
                    dict(
                    scene_id=scene_id,
                    frame_token=sample_idx,
                    timestamp=timestamp,
            ...
            with open(result_path, 'w', encoding='utf-8') as file:
                json.dump(generated_text, file, ensure_ascii=False, indent=2)
