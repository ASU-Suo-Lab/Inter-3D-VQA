...
from opendrivevla_v5.config.common import DEFAULT_DATASET_VERSION, resolve_dataset_version_paths
from opendrivevla_v5.utils.io import ensure, load_json


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate OpenDriveVLA Intersection V5 predictions.")
    parser.add_argument("--dataset-version", choices=["v5", "v6"], default=DEFAULT_DATASET_VERSION)
    parser.add_argument("--predictions", default=None, help="Merged predictions JSONL.")
    parser.add_argument("--pred-dir", default=None, help="Optional directory of JSON prediction fragments.")
...
def main() -> None:
    ...
    if args.predictions:
        predictions_jsonl = Path(args.predictions).resolve()
        ensure(predictions_jsonl.is_file(), f"Predictions JSONL not found: {predictions_jsonl}")
    else:
        ensure(args.pred_dir is not None, "Either --predictions or --pred-dir must be provided.")
