...
def load_uniad_config_dict(config_path: Optional[Union[str, Path]] = None):
    repo_root = Path(__file__).resolve().parents[3]
    if config_path is None:
        config_path = repo_root / "projects" / "configs" / "stage1_track_map" / "base_track_map.py"
    else:
        config_path = Path(config_path).resolve()
    namespace = runpy.run_path(str(config_path), init_globals={"__file__": str(config_path)})
    return {key: namespace[key] for key in ("plugin", "plugin_dir", "model", "test_cfg") if key in namespace}
...
class UniadTrackMapVisionTower(nn.Module):
    def __init__(self, vision_tower, vision_tower_cfg, delay_load=False):
        ...
        self.uniad_config_path = getattr(vision_tower_cfg, "uniad_config_path", None)
        uniad_config_dict = load_uniad_config_dict(self.uniad_config_path)
        ...
        if not delay_load:
            ...
            if self.uniad_config_path:
                rank0_print(f"Using UniAD config: {self.uniad_config_path}")
            self.load_model()
