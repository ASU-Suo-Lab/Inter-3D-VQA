...
from opendrivevla_v5.utils.tensors import move_data_to_device, remove_datacontainer
...
def custom_single_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    """Test model with multiple gpus.
    This method tests model with multiple gpus and collects the results
    under two different modes: gpu and cpu modes. By setting 'gpu_collect=True'
    it encodes results to gpu tensors and use gpu communication for results
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            result, results_for_vlm = model(return_loss=False, rescale=True, **data)
