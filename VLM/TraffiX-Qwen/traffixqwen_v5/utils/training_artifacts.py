...
from traffixqwen_v5.data.common import dump_json, ensure, load_json
...
def extract_history(trainer_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for raw_event in raw_events:
        ...
        if isinstance(raw_event.get("loss"), (int, float)):
            ...
            if isinstance(raw_event.get("grad_norm"), (int, float)):
                event["grad_norm"] = float(raw_event["grad_norm"])
    ...
    train_summary = next(
        (
            ...
            if isinstance(raw_event, dict) and isinstance(raw_event.get("train_loss"), (int, float))
        ),
        None,
    )
    return {
...
def _plot_history(train_events: List[Dict[str, Any]], eval_events: List[Dict[str, Any]], plot_path: Path) -> None:
    ...
    if train_events:
        ...
        ax2.plot(
            [event["step"] for event in train_events],
            [event.get("lr", 0.0) for event in train_events],
            label="lr",
            color="#2ca02c",
            linewidth=1.0,
            alpha=0.7,
