...
def _resolve_lora_base_path(model_path: str, model_base: str | None) -> str | None:
    ...
    if os.path.isfile(adapter_config_path):
        ...
        if isinstance(candidate, str) and candidate.strip() and os.path.exists(candidate):
            return candidate
...
def _normalize_llava_qwen_config(config):
    config.model_type = "llava_qwen"
    config.architectures = ["LlavaQwenForCausalLM"]
    return config
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is None:
            model_base = _resolve_lora_base_path(model_path, model_base)
            if model_base is not None:
                rank0_print(f"Resolved LoRA base model from checkpoint metadata: {model_base}")
            else:
                warnings.warn(
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                lora_cfg_pretrained = _normalize_llava_qwen_config(LlavaQwenConfig.from_pretrained(model_path))
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                # breakpoint()
                ...
                model = LlavaQwenForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=False, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            else:
            ...
            if "mixtral" in model_name.lower():
                ...
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                llava_cfg = _normalize_llava_qwen_config(LlavaQwenConfig.from_pretrained(model_path))
                if overwrite_config is not None:
                    rank0_print(f"Overwriting config with {overwrite_config}")
                    for k, v in overwrite_config.items():
            ...
            mm_projector_weights = {
                key: value.to(model_dtype) if torch.is_tensor(value) and value.is_floating_point() else value
                for key, value in mm_projector_weights.items()
            }
            incompatible_keys = model.load_state_dict(mm_projector_weights, strict=False)
            rank0_print(
                f"Loaded projector-only checkpoint from {model_path}. Incompatible keys: {incompatible_keys}"
            )
        else:
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if vision_tower is None:
            raise ValueError(
                "Loaded a multimodal model without a vision tower. "
                f"model_path={model_path}, model_base={model_base}, model_name={model_name}, "
                f"config_model_type={getattr(model.config, 'model_type', None)}, "
                f"mm_vision_tower={getattr(model.config, 'mm_vision_tower', None)}"
            )
