...
from llava.train.llava_trainer import LLaVATrainer, save_model_metadata
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    ...
    if check_only_save_mm_adapter_tunnable:
        ...
        save_model_metadata(trainer.model, output_dir)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if training_args.lora_enable:
            for name, param in model.named_parameters():
                if "lora_" in name:
                    param.requires_grad_(True)
            rank0_print("Keeping LoRA adapter parameters trainable while the base language model stays frozen.")
    ...
    if training_args.lora_enable:
        ...
        if training_args.local_rank == 0 or training_args.local_rank == -1:
            save_model_metadata(model, training_args.output_dir)
