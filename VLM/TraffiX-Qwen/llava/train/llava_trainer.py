...
def resolve_config_owner(model):
    candidates = []
    if hasattr(model, "get_base_model"):
        ...
        if base_model is not None:
            candidates.extend([base_model, getattr(base_model, "model", None)])

    candidates.extend([model, getattr(model, "model", None)])

    for candidate in candidates:
        if candidate is None or not hasattr(candidate, "config"):
            continue
        if getattr(candidate.config, "mm_vision_tower", None) is not None:
            return candidate

    return model


def save_model_metadata(model, output_dir: str) -> None:
    config_owner = resolve_config_owner(model)
    config = getattr(config_owner, "config", None) or getattr(model, "config", None)
    generation_config = getattr(config_owner, "generation_config", None) or getattr(model, "generation_config", None)
...
def save_lora_checkpoint(model, output_dir: str, lora_bias: str) -> None:
    state_dict = get_peft_state_maybe_zero_3(model.named_parameters(), lora_bias)
    non_lora_state_dict = get_peft_state_non_lora_maybe_zero_3(model.named_parameters())
    save_model_metadata(model, output_dir)
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "lora_enable", False):
            ...
            if self.args.local_rank == 0 or self.args.local_rank == -1:
                save_lora_checkpoint(self.model, output_dir, self.args.lora_bias)
