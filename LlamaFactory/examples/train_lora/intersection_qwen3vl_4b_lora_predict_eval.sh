...
default_adapter_path_for_version() {
  ...
  case "${dataset_version}" in
    v5)
      printf '%s\n' "${LLAMAFACTORY_DIR}/saves/intersection_qwen3-vl-4b/lora/sft_v5"
      ;;
    v6)
      printf '%s\n' "${LLAMAFACTORY_DIR}/saves/intersection_qwen3-vl-4b/lora/sft_v6"
      ;;
    *)
      echo "Unsupported dataset_version: ${dataset_version}. Use v5 or v6." >&2
      exit 1
...
default_predict_output_dir_for_version() {
  ...
  case "${dataset_version}" in
    v5)
      printf '%s\n' "${LLAMAFACTORY_DIR}/saves/intersection_qwen3-vl-4b/lora/predict_v5"
      ;;
    v6)
      printf '%s\n' "${LLAMAFACTORY_DIR}/saves/intersection_qwen3-vl-4b/lora/predict_v6"
      ;;
    *)
      echo "Unsupported dataset_version: ${dataset_version}. Use v5 or v6." >&2
      exit 1
...
if [[ $# -ge 3 && -n "${3}" ]]; then
  ...
  EVAL_OUTPUT_DIR="${LLAMAFACTORY_DIR}/eval/${DATASET_PREFIX}_qwen3vl_4b"
fi
...
bash "${EVAL_SCRIPT}" "${GENERATED_PREDICTIONS}" "${EVAL_OUTPUT_DIR}" "${SPLIT}" "${DATASET_VERSION}" "${DATASET_DIR}" --model-tag qwen3vl_4b "${NORMALIZED_EVAL_ARGS[@]}"
