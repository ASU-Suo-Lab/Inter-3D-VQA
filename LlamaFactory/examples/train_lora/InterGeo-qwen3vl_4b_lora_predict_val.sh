...
ADAPTER_DIR="${LLAMAFACTORY_DIR}/saves/InterGeo-qwen3vl_4b/${DATASET_VERSION}/train"
OUTPUT_DIR="${LLAMAFACTORY_DIR}/saves/InterGeo-qwen3vl_4b/${DATASET_VERSION}/predict"
...
val_path = dataset_dir / f"{dataset_prefix}_val.jsonl"
if not any(line.strip() for line in val_path.open("r", encoding="utf-8")):
    raise SystemExit(f"{val_path.name}: dataset is empty.")
PY

cd "${LLAMAFACTORY_DIR}"
llamafactory-cli train examples/train_lora/InterGeo-qwen3vl_4b_lora_predict_val.yaml \
  adapter_name_or_path="${ADAPTER_DIR#${LLAMAFACTORY_DIR}/}" \
