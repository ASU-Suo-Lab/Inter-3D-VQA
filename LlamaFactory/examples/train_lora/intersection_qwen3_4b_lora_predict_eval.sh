...
if [[ -n "${ADAPTER_ARG}" ]]; then
  ...
  ADAPTER_PATH="${LLAMAFACTORY_DIR}/saves/intersection_qwen3-4b/lora/sft_v5"
fi

if [[ -n "${PREDICT_OUTPUT_ARG}" ]]; then
  PREDICT_OUTPUT_DIR="$(resolve_lf_path "${PREDICT_OUTPUT_ARG}")"
else
  PREDICT_OUTPUT_DIR="${LLAMAFACTORY_DIR}/saves/intersection_qwen3-4b/lora/predict_v5"
fi

if [[ -n "${EVAL_OUTPUT_ARG}" ]]; then
  EVAL_OUTPUT_DIR="$(resolve_lf_path "${EVAL_OUTPUT_ARG}")"
else
  EVAL_OUTPUT_DIR="${LLAMAFACTORY_DIR}/eval/intersection_vqa_text_v5_qwen3_4b"
fi
...
bash "${EVAL_SCRIPT}" "${GENERATED_PREDICTIONS}" "${EVAL_OUTPUT_DIR}" "${SPLIT}" "${DATASET_DIR}" --model-tag qwen3_4b
