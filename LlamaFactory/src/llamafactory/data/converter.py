...
class DatasetConverter:
    ...
    def _find_lidar_object_features(self, lidar_object_features: str | list[str] | None) -> list[str] | None:
        if lidar_object_features is None:
            return None
        elif not isinstance(lidar_object_features, list):
            lidar_object_features = [lidar_object_features]
        elif len(lidar_object_features) == 0:
            return None
        else:
            lidar_object_features = lidar_object_features[:]

        if self.dataset_attr.load_from in ["script", "file"]:
            for i in range(len(lidar_object_features)):
                object_feature_path = os.path.join(self.data_args.media_dir, lidar_object_features[i])
                if os.path.isfile(object_feature_path):
                    lidar_object_features[i] = object_feature_path
                else:
                    logger.warning_rank0_once(
                        f"LiDAR object feature {lidar_object_features[i]} does not exist in `media_dir`. Use original path."
                    )

        return lidar_object_features

    def _find_lidar_object_geometry_features(
        self, lidar_object_geometry_features: str | list[str] | None
    ) -> list[str] | None:
        if lidar_object_geometry_features is None:
            return None
        elif not isinstance(lidar_object_geometry_features, list):
            lidar_object_geometry_features = [lidar_object_geometry_features]
        elif len(lidar_object_geometry_features) == 0:
            return None
        else:
            lidar_object_geometry_features = lidar_object_geometry_features[:]

        if self.dataset_attr.load_from in ["script", "file"]:
            for i in range(len(lidar_object_geometry_features)):
                object_geometry_feature_path = os.path.join(self.data_args.media_dir, lidar_object_geometry_features[i])
                if os.path.isfile(object_geometry_feature_path):
                    lidar_object_geometry_features[i] = object_geometry_feature_path
                else:
                    logger.warning_rank0_once(
                        f"LiDAR object geometry feature {lidar_object_geometry_features[i]} does not exist in `media_dir`. Use original path."
                    )

        return lidar_object_geometry_features

    def _find_lidar_scene_features(self, lidar_scene_features: str | list[str] | None) -> list[str] | None:
        if lidar_scene_features is None:
            return None
        elif not isinstance(lidar_scene_features, list):
            lidar_scene_features = [lidar_scene_features]
        elif len(lidar_scene_features) == 0:
            return None
        else:
            lidar_scene_features = lidar_scene_features[:]

        if self.dataset_attr.load_from in ["script", "file"]:
            for i in range(len(lidar_scene_features)):
                scene_feature_path = os.path.join(self.data_args.media_dir, lidar_scene_features[i])
                if os.path.isfile(scene_feature_path):
                    lidar_scene_features[i] = scene_feature_path
                else:
                    logger.warning_rank0_once(
                        f"LiDAR scene feature {lidar_scene_features[i]} does not exist in `media_dir`. Use original path."
                    )

        return lidar_scene_features
...
class AlpacaDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        output = {
            ...
            "_lidar_object_features": self._find_lidar_object_features(example.get(self.dataset_attr.lidar_object_features))
            if self.dataset_attr.lidar_object_features
            else None,
            "_lidar_object_geometry_features": self._find_lidar_object_geometry_features(
                example.get(self.dataset_attr.lidar_object_geometry_features)
            )
            if self.dataset_attr.lidar_object_geometry_features
            else None,
            "_lidar_scene_features": self._find_lidar_scene_features(example.get(self.dataset_attr.lidar_scene_features))
            if self.dataset_attr.lidar_scene_features
            else None,
        }
        return output
